...
BOOL UsingListboxDlg::Cls_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam) 
{
	...
	myVector.push_back(L"Милан Италия");
	myVector.push_back(L"Ливерпуль Англия");
	myVector.push_back(L"Бавария Германия");
	myVector.push_back(L"Барселона Испания");
	myVector.push_back(L"Арсенал Англия");
	myVector.push_back(L"Лион Франция");
	myVector.push_back(L"Интер Италия");
	myVector.push_back(L"Реал Испания");
	for (int i = 0; i < myVector.size(); i++) {
		TCHAR buffer[MAX_PATH];
		lstrcpy(buffer, myVector[i].c_str());
		// Добавляем скопированную строку в начало списка
		SendMessage(hList1, LB_INSERTSTRING, i, reinterpret_cast<LPARAM>(buffer));
	}
...
void UsingListboxDlg::Cls_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case IDC_LIST1:
	{
		...
		if (index != LB_ERR) 
		{
			...
			SetWindowText(hEdit3, myVector[index].c_str()); 
	...
	case IDC_EDITBUTTON:
	{
		...
		if (index != LB_ERR)
		{
			...
			myVector[index] = pBuffer;
			
			SendMessage(hList1, LB_DELETESTRING, index, 0);
			SendMessage(hList1, LB_INSERTSTRING, index, reinterpret_cast<LPARAM>(pBuffer));
